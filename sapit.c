/*
cc -O sapit.c -o sapit util/aa/cones.o util/aa/stutest.o util/aa/bestrot.o util/wt/util.o util/wt/geom.o util/wt/sort.o util/aa/pdbprot.o util/aa/matrix.o util/aa/siva.o util/aa/ql.o util/eigen.o -lm
...
float	score, acut, cyc_no = (float)cycle;
	...
	supstr(a, b, seqa, seqb, sim, aln, len);
	for (i=1; i<=lenb; i++) {
		for (j=1; j<=lena; j++) {
			bias[i][j] *= BIAS_DAMP;
		}
...
supstr (stra, strb, seqa, seqb, sim, aln, len)
Tri	**stra, **strb;
Seq	*seqa, *seqb;
float	**sim;
int	**aln, len;
...
int	na = seqa->len;
int	nb = seqb->len;
float	cut, rms, pct, suw, sulg, sud1, sud2, sud3, sum = 0.0;
float	sumrms = 0.0;
int	i, j, n, id;
	sa = (float*)alloca(sizeof(float)*(na+2));
	for (i=1; i<=na; i++) sa[i] = 0.0;
	...
	sulg = sud1 = sud2 = sud3 = 0.0;
	for (i=len; i>0; i--) 
	{ int	a = aln[0][i], b = aln[1][i];
	  float d = vdif(seqa->ca[a],seqb->ca[b]);
		sud1 += exp(-(d/6.)*(d/6.));
		sud2 += exp(-(d/7.)*(d/7.)); /* close to sulg */
		sud3 += exp(-(d/8.)*(d/8.));
		d /= 5.0; d *= d; d+= 1.0;
		sulg += 1.0/d;          /* Levitt+Gerstein 98 */
	}	
	sud1 = 100*sud1/(float)len;
	sud2 = 100*sud2/(float)len;
	sud3 = 100*sud3/(float)len;
	sulg = 100*sulg/(float)len;
	printf("%s Weighted RMSd = %7.3f over %d atoms %f %f %5.1f %f %f %f %f\n",
		pdbcode, rms, len, suw, sum, pct, sulg, sud1, sud2, sud3);
	...
	rms = supermac(ww,va,vb,len,ac,bc,rot);
	printf("%s Un-weighted RMSd = %7.3f over all %d matched atoms %f %f  %5.1f %f %f %f %f\n",
		pdbcode, rms, n, suw, sum, pct, sulg, sud1, sud2, sud3);
	return rms;
}
