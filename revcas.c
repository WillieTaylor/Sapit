...
float	apct, bpct;
	...
	sec = (float*)alloca(sizeof(float)*m*2);
	swt = (float*)alloca(sizeof(float)*m*2);
	acs = (float*)alloca(sizeof(float)*m+2);
	rid = (int*)alloca(sizeof(int)*m*2);
	...
	{ Atom_ *Atoms = prot->Chains[0].Atoms;
               	...
               	nat[i+1].y = Atoms[i].Y;
               	nat[i+1].z = Atoms[i].Z;
               	sec[i+1] = Atoms[i].Occu;
               	acc[i+1] = Atoms[i].Bfact;
		res[i+1] = Atoms[i].Aa;
		rid[i+1] = i+1;
	}
	secs = (char*)alloca(sizeof(char)*m*2);
	consec_seen(prot->Chains,secs,&apct,&bpct);
	Pr(apct) Pr(bpct) NL
	mode = *argv[2];
	if (islower(mode)) {
		mode = toupper(mode);
	...
	nout = nres+1;
	sec[0] = sec[nout] = sec[nout+1] = 0.0;
	acc[0] = acc[nout] = acc[nout+1] = 0.0;
	res[0] = res[nout] = res[nout+1] = 'B';
	rid[0] = 0; rid[nout] = nres+1; rid[nout+1] = 0;
	for (i=1; i<=nout; i++) {
		sec[i] =  0.0;
		if (secs[i]=='E') sec[i] = 1.0; // beta
		if (secs[i]=='H') sec[i] = 2.0; // alpha
		swt[i] = -1.0;
		if (secs[i]=='E') swt[i] = 3.0; // beta
		if (secs[i]=='H') swt[i] = 2.0; // alpha
	}
	for (i=1; i<=nout; i++) {
	...
	for (i=0; i<=nres+1 && mode=='N'; i++) {
		fprintf(out,"ATOM%7d  CA  %3s A%4d    %8.3f%8.3f%8.3f %5.2f %5.2f\n",
               		i, aa_code13(res[i]), i, nat[i].x, nat[i].y, nat[i].z, acc[i], sec[i]);
	}
	for (i=nres+1; i>=0 && mode=='R'; i--) {
		fprintf(out,"ATOM%7d  CA  %3s A%4d    %8.3f%8.3f%8.3f %5.2f %5.2f\n",
               		i, aa_code13(res[i]), nres-i+1, nat[i].x, nat[i].y, nat[i].z, acc[i], sec[i]);
	}
	fprintf(out,"TER\n");
	fclose(out);
	out = fopen("test.out","w");
	for (i=1; i<=nres && mode=='N'; i++) {
		fprintf(out,"ATOM%7d  CA  %3s Z%4d    %8.3f%8.3f%8.3f %5.2f %5.2f\n",
               		i, aa_code13(res[i]), i, nat[i].x, nat[i].y, nat[i].z, acc[i], sec[i]);
	}
	...
	n = 0;
	for (j=2; j<nres-1; j++) {
		acs[n] =  2.0*acc[j-1]+acc[j]+acc[j+1]+acc[j+2]*2.0  //exposure
			+ 0.5*swt[j-1]+swt[j]+swt[j+1]+swt[j+2]*0.5; // sec.str
		n++;
	}
	sort(0,acs,0,at,n,1);
	m = 1;
	for (i=0; i<=nres+1; i++) in[i] = 0;
	for (i=n-1; i>=0; i--) { int skip, rati = at[i]+3;
		if (in[rati]<0) continue;
		if (acs[at[i]]>0.5) continue;
		skip = 0;
		for (j=rati-3; j<=rati+4; j++) {
			if (in[j]>0) skip = 1;
		}
		if (skip) continue; // too close to a used location
		for (j=rati-3; j<=rati+4; j++) in[j] = -1; // mark location
		in[rati] = m++;
	}
	nout = 0;
	for (j=2; j<nres-1; j++)
	{ char	c; int k;
		...
		for (i=j+1, k=1; i<=j+nres && mode=='N'; i++, k++) {
			fprintf(out,"ATOM%7d  CA  %3s %c%4d    %8.3f%8.3f%8.3f %5.2f %5.2f\n",
                		rid[i], aa_code13(res[i]), c, k, nat[i].x, nat[i].y, nat[i].z, acc[i], sec[i]);
		}
	...
	{float	nn = 0.5*(float)n,
		...
		for (j=Nter+1; j<Cter-2; j++)
			...
			for (k=j-4; k<=j+4; k++) { float d = vdif(cb[j],cb[k]);
				if (d<7.0) setdist(ca,work,d,0.5,j,k);
			}
        	...
        	for (j=1; j<len-1; j++) {
        		for (k=1; k<=len; k++) vcopy(ca[k],work+k);
			for (k=j+2; k<=len; k++) {
				if (vddif(ca[j],ca[k])<20.0) setdist(ca,work,4.5,0.5,j,k);
			}
        		for (k=1; k<=len; k++) vcopy(work[k],ca+k);
		}
	}
...
Vec	*work;
	...
	for (i=0; i<m; i++) {
        	for (j=1; j<=len; j++) vcopy(ca[j],work+j);
        	for (j=1; j< len; j++) setdist(ca,work,3.8,0.5,j,j+1);
        	for (j=1; j<=len; j++) vcopy(work[j],ca+j);
	}
}
...
float     *psec, **mat;
	...
	*pcta = *pctb = 0.0;
        for (i=1; i<len; i++) {
		if (secs[i]=='E' && secs[i-1]!='E' && secs[i+1]!='E') secs[i] = '-';
		if (secs[i]=='H' && secs[i-1]!='H' && secs[i+1]!='H') secs[i] = '-';
