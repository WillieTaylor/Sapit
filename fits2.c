/*
cc fits2.c -o fits2 util/wt/util.o util/wt/sort.o -lm
...
written in sapit.c
        ...
        printf("%s Un-weighted RMSd = %7.3f over all %d matched atoms %f %f  %5.1f %f %f %f %f\n",
                pdbcode, rms, n, suw, sum, pct, sulg, sud1, sud2, sud3);
                          5   8        12        14    15    16    17 
rewritten in sapit.csh
	awk '{print $8, $5, $12, $14, $15, $16, $17}' rms.dat > nat1rev2.dat (++ --> random.dat)
read
	sscanf(line,"%d %f %f %f %f %f %f", &i, &r, &s0, &t0, &t1, &t2, &t3);
                                             n  rms sum sulg sud1 sud2 sud3					   
*/
...
FILE *dat, *plt;
	...
	nmax = 0;
	mave = 0.0;
	while (1) // read random.dat to mean alignment length (i)
...
Pi(naln) Pi(maln) Pi(nmax) Pi(n) Pr(mave)
	...
	n = 0;
	nave = 0.0;
	while (1) // read random.dat again to split high/low sets
	{ float s0, t0, t1, t2, t3;
	  int	io = read_line(dat,line);
		if (io<=0) break;
		sscanf(line,"%d %f %f %f %f %f %f", &i, &r, &s0, &t0, &t1, &t2, &t3);
...
Pi(n) Pr(lave) Pr(nave) NL
	...
	n = 0;
	dat = fopen(file,"r");
	while (1) // read file again to get values
...
Pr(sum) Pr(wsum) NL
	// d[] holds the (transformed) RMS values,
	// w[] are weights = 1 if aln.len > mean (nave) else damped
	sort(0,d,0,p,n,1);
	ssum = 0.0;
	for (i=n-1; i>-1; i--) {
		set = d[p[i]];
...
*/
	...
	if (EVD) {
		for (i=0; i<N; i++) {
			...
			x=(float)i-bopt; g[i] = 10.0*exp(-x*x/cc);
		}
        	h[0] = g[0];
        	for (i=1; i<N; i++) h[i] = g[i]+h[i-1];
	}
...
Pr(y) NL
	...
	z = y/z;
	Pr(z) Pr(zmax) NL
	x = 1.0/(copt*0.005)*log(-log(.5))+bopt;
	Pr(x) NL
	s = smin+(x-50.0)*range/100;
	Pr(s) NL
}
